...
__export(src_exports, {
  ...
  BeamsError: () => BeamsError,
  BeamsErrorCode: () => BeamsErrorCode,
  BeamsHubClient: () => BeamsHubClient,
  BeamsHubTxFactory: () => BeamsHubTxFactory,
  BeamsSubgraphClient: () => BeamsSubgraphClient,
  CallerClient: () => CallerClient,
  ERC20TxFactory: () => ERC20TxFactory,
  ImmutableSplitsDriverClient: () => ImmutableSplitsDriverClient,
...
var formatBeamsReceivers = (receivers) => {
  const uniqueReceivers = receivers.reduce((unique, o) => {
    if (!unique.some(
      (obj) => obj.userId === o.userId && import_ethers.BigNumber.from(obj.config).eq(import_ethers.BigNumber.from(o.config))
    )) {
      unique.push(o);
    }
    return unique;
  ...
  const sortedReceivers = uniqueReceivers.sort(
    (a, b) => import_ethers.BigNumber.from(a.userId).gt(import_ethers.BigNumber.from(b.userId)) ? 1 : import_ethers.BigNumber.from(a.userId).lt(import_ethers.BigNumber.from(b.userId)) ? -1 : import_ethers.BigNumber.from(a.config).gt(import_ethers.BigNumber.from(b.config)) ? 1 : import_ethers.BigNumber.from(a.config).lt(import_ethers.BigNumber.from(b.config)) ? -1 : 0
  );
...
var formatSplitReceivers = (receivers) => {
  ...
  const sortedReceivers = uniqueReceivers.sort(
    (a, b) => import_ethers.BigNumber.from(a.userId).gt(import_ethers.BigNumber.from(b.userId)) ? 1 : import_ethers.BigNumber.from(a.userId).lt(import_ethers.BigNumber.from(b.userId)) ? -1 : 0
  );
  return sortedReceivers;
};
...
var safeBeamsTx = (tx) => {
  if (isNullOrUndefined(tx.value)) {
    tx.value = import_ethers.BigNumber.from(0);
  }
  return tx;
};
...
var BeamsHub__factory = class {
  ...
  static connect(address, signerOrProvider) {
    return new import_ethers3.Contract(address, _abi2, signerOrProvider);
  }
};
...
var Caller__factory = class {
  ...
  static connect(address, signerOrProvider) {
    return new import_ethers4.Contract(address, _abi3, signerOrProvider);
  }
};
...
var IERC20__factory = class {
  ...
  static connect(address, signerOrProvider) {
    return new import_ethers5.Contract(address, _abi4, signerOrProvider);
  }
};
...
var ImmutableSplitsDriver__factory = class {
  ...
  static connect(address, signerOrProvider) {
    return new import_ethers6.Contract(
      address,
      _abi5,
      signerOrProvider
    );
...
var NFTDriver__factory = class {
  ...
  static connect(address, signerOrProvider) {
    return new import_ethers7.Contract(address, _abi6, signerOrProvider);
  }
};
...
var validateAddress = (address) => {
  if (!import_ethers8.ethers.utils.isAddress(address)) {
...
((Utils2) => {
  let Metadata;
  ((Metadata2) => {
    Metadata2.keyFromString = (key) => import_ethers9.ethers.utils.formatBytes32String(key);
    Metadata2.valueFromString = (value) => import_ethers9.ethers.utils.hexlify(import_ethers9.ethers.utils.toUtf8Bytes(value));
    ...
    Metadata2.convertMetadataBytesToString = (userMetadata) => {
      if (!import_ethers9.ethers.utils.isBytesLike(userMetadata == null ? void 0 : userMetadata.key) || !import_ethers9.ethers.utils.isBytesLike(userMetadata == null ? void 0 : userMetadata.value)) {
        throw BeamsErrors.argumentError(
          `Invalid key-value user metadata pair: key or value is not a valid BytesLike object.`
        );
      }
      return {
        key: import_ethers9.ethers.utils.parseBytes32String(userMetadata.key),
        value: import_ethers9.ethers.utils.toUtf8String(userMetadata.value)
      };
    };
  ...
  ((Network2) => {
    Network2.configs = {
      ...
      80001: {
        ...
        IMMUTABLE_SPLITS_DRIVER_ADMIN: "0x840C1b6ce85bBFEbcFAd737514c0097B078a7E7E",
        IMMUTABLE_SPLITS_DRIVER_ID: "2",
        SUBGRAPH_URL: "https://api.studio.thegraph.com/query/50446/beaaam-mumbai/v0.0.8"
      }
    };
  ...
  let Asset;
  ((Asset2) => {
    Asset2.getAddressFromId = (assetId) => import_ethers9.ethers.utils.getAddress(import_ethers9.BigNumber.from(assetId).toHexString());
    Asset2.getIdFromAddress = (tokenAddress) => {
      validateAddress(tokenAddress);
      return import_ethers9.BigNumber.from(import_ethers9.ethers.utils.getAddress(tokenAddress)).toBigInt();
    };
  ...
  ((BeamsReceiverConfiguration2) => {
    BeamsReceiverConfiguration2.toUint256 = (beamsReceiverConfig) => {
      ...
      const { beamId, start, duration, amountPerSec } = beamsReceiverConfig;
      let config = import_ethers9.BigNumber.from(beamId);
    ...
    BeamsReceiverConfiguration2.fromUint256 = (beamsReceiverConfig) => {
      const configAsBn = import_ethers9.BigNumber.from(beamsReceiverConfig);
      const beamId = configAsBn.shr(160 + 32 + 32);
      const amountPerSec = configAsBn.shr(32 + 32).and(import_ethers9.BigNumber.from(1).shl(160).sub(1));
      const start = configAsBn.shr(32).and(import_ethers9.BigNumber.from(1).shl(32).sub(1));
      const duration = configAsBn.and(import_ethers9.BigNumber.from(1).shl(32).sub(1));
      const config = {
    ...
    static async createNewStreamFlow(payload) {
      ...
      validateSetBeamsInput(
        ...
        currentReceivers == null ? void 0 : currentReceivers.map((r) => ({
          ...
          config: utils_default.BeamsReceiverConfiguration.fromUint256(import_ethers10.BigNumber.from(r.config).toBigInt())
        })),
  ...
  class Presets {
    static async createNewStreamFlow(payload) {
      ...
      validateSetBeamsInput(
        ...
        currentReceivers == null ? void 0 : currentReceivers.map((r) => ({
          userId: r.userId.toString(),
          config: utils_default.BeamsReceiverConfiguration.fromUint256(import_ethers11.BigNumber.from(r.config).toBigInt())
        })),
        newReceivers == null ? void 0 : newReceivers.map((r) => ({
          userId: r.userId.toString(),
          config: utils_default.BeamsReceiverConfiguration.fromUint256(import_ethers11.BigNumber.from(r.config).toBigInt())
...
var _AddressDriverClient = class {
  ...
  approve(tokenAddress) {
    ...
    validateAddress(tokenAddress);
    const signerAsErc20Contract = IERC20__factory.connect(tokenAddress, __privateGet(this, _signer3));
    return signerAsErc20Contract.approve(__privateGet(this, _driverAddress4), import_ethers12.constants.MaxUint256);
  }
  async getUserId() {
    ensureSignerExists(__privateGet(this, _signer3));
    const signerAddress = await __privateGet(this, _signer3).getAddress();
  ...
  async give(receiverUserId, tokenAddress, amount) {
    ...
    validateAddress(tokenAddress);
    if (!amount || import_ethers12.BigNumber.from(amount).lte(0)) {
      throw BeamsErrors.argumentError(
  ...
  async setBeams(tokenAddress, currentReceivers, newReceivers, transferToAddress, balanceDelta = 0) {
    ...
    validateSetBeamsInput(
      ...
      currentReceivers == null ? void 0 : currentReceivers.map((r) => ({
        userId: r.userId.toString(),
        config: utils_default.BeamsReceiverConfiguration.fromUint256(import_ethers12.BigNumber.from(r.config).toBigInt())
      })),
      newReceivers == null ? void 0 : newReceivers.map((r) => ({
        userId: r.userId.toString(),
        config: utils_default.BeamsReceiverConfiguration.fromUint256(import_ethers12.BigNumber.from(r.config).toBigInt())
...
AddressDriverClient.getUserAddress = (userId) => {
  ...
  const userIdAsBn = import_ethers12.ethers.BigNumber.from(userId);
  if (userIdAsBn.lt(0) || userIdAsBn.gt(import_ethers12.ethers.constants.MaxUint256)) {
    throw BeamsErrors.argumentError(
      `Could not get user address: ${userId} is not a valid positive number within the range of a uint256.`
    );
  }
  const mid64BitsMask = import_ethers12.ethers.BigNumber.from(2).pow(64).sub(1).shl(160);
  ...
  const mask = import_ethers12.ethers.BigNumber.from(2).pow(160).sub(1);
  const address = userIdAsBn.and(mask).toHexString();
  const paddedAddress = import_ethers12.ethers.utils.hexZeroPad(address, 20);
  return import_ethers12.ethers.utils.getAddress(paddedAddress);
};
...
var _BeamsHubClient = class {
  ...
  async getBeamsBalanceAt(userId, tokenAddress, receivers, timestamp) {
    ...
    validateBeamsReceivers(
      receivers.map((r) => ({
        userId: r.userId.toString(),
        config: utils_default.BeamsReceiverConfiguration.fromUint256(import_ethers13.BigNumber.from(r.config).toBigInt())
      }))
    );
    if (isNullOrUndefined(userId)) {
      throw BeamsErrors.argumentMissingError(
...
var constants2 = {
  ...
  ASSOCIATED_APP_KEY,
  ASSOCIATED_APP_KEY_BYTES: import_ethers14.ethers.utils.formatBytes32String(ASSOCIATED_APP_KEY)
};
...
var _BeamsSubgraphClient = class {
  ...
  async getNftSubAccountsByOwner(ownerAddress, skip = 0, first = 100) {
    ...
    const response = await this.query(getNftSubAccountsByOwner, {
      ownerAddress: import_ethers15.ethers.utils.getAddress(ownerAddress),
      skip,
      first
    });
    ...
    return nftSubAccounts ? nftSubAccounts.map((s) => ({
      tokenId: s.id,
      ownerAddress: import_ethers15.ethers.utils.getAddress(s.ownerAddress)
    })) : [];
  }
  async getNftSubAccountOwnerByTokenId(tokenId) {
    ...
    return nftSubAccount ? { tokenId: nftSubAccount.id, ownerAddress: import_ethers15.ethers.utils.getAddress(nftSubAccount.ownerAddress) } : null;
  }
  ...
  async getArgsForSqueezingAllBeams(userId, senderId, tokenAddress) {
    ...
    const filtered = allBeamsSetEvents.reduce((unique, o) => {
      ...
      return unique;
    }, []).filter((e) => e.assetId === utils_default.Asset.getIdFromAddress(import_ethers15.ethers.utils.getAddress(tokenAddress)));
    ...
    const historyHash = ((_a = beamsSetEventsToSqueeze[beamsSetEventsToSqueeze.length - 1]) == null ? void 0 : _a.beamsHistoryHash) || import_ethers15.ethers.constants.HashZero;
    const beamsHistory = beamsSetEventsToSqueeze == null ? void 0 : beamsSetEventsToSqueeze.map((beamsSetEvent) => {
      ...
      const historyItem = {
        beamsHash: shouldSqueeze ? import_ethers15.ethers.constants.HashZero : beamsSetEvent.receiversHash,
...
var _NFTDriverClient = class {
  ...
  approve(tokenAddress) {
    validateAddress(tokenAddress);
    const signerAsErc20Contract = IERC20__factory.connect(tokenAddress, __privateGet(this, _signer7));
    return signerAsErc20Contract.approve(__privateGet(this, _driverAddress6), import_ethers16.constants.MaxUint256);
  }
  ...
  async give(tokenId, receiverUserId, tokenAddress, amount) {
    ...
    if (!amount || import_ethers16.BigNumber.from(amount).lte(0)) {
      throw BeamsErrors.argumentError(
        `Could not give: '${nameOf({ amount })}' must be greater than 0.`,
        nameOf({ amount }),
        amount
  ...
  async setBeams(tokenId, tokenAddress, currentReceivers, newReceivers, transferToAddress, balanceDelta = 0) {
    ...
    validateSetBeamsInput(
      ...
      currentReceivers == null ? void 0 : currentReceivers.map((r) => ({
        userId: r.userId.toString(),
        config: utils_default.BeamsReceiverConfiguration.fromUint256(import_ethers16.BigNumber.from(r.config).toBigInt())
      })),
      newReceivers == null ? void 0 : newReceivers.map((r) => ({
        userId: r.userId.toString(),
        config: utils_default.BeamsReceiverConfiguration.fromUint256(import_ethers16.BigNumber.from(r.config).toBigInt())
