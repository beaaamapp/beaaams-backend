...
import type { BeamsReceiverStruct, SplitsReceiverStruct } from './types';
...
/** @internal */
export const formatBeamsReceivers = (receivers: BeamsReceiverStruct[]) => {
	// Beams receivers must be sorted by user ID and config, deduplicated, and without amount per second <= 0.

	const uniqueReceivers = receivers.reduce((unique: BeamsReceiverStruct[], o) => {
		if (
			!unique.some(
				(obj: BeamsReceiverStruct) => obj.userId === o.userId && BigNumber.from(obj.config).eq(BigNumber.from(o.config))
			)
		) {
			unique.push(o);
		}
...
/**
 * Source: https://github.com/radicle-dev/beams-app-v2/blob/main/src/lib/utils/expect.ts
 *
...
/** @internal */
export const safeBeamsTx = (tx: PopulatedTransaction): PopulatedTransaction => {
